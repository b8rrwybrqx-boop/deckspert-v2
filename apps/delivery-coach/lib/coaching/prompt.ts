...
export function buildCoachingPrompt(input: BuildPromptInput) {
  ...
  return [
    ...
    '{ "executiveSummary": string, "overallScore": number, "dimensionScores": { "voicePacing": number, "presenceConfidence": number, "bodyLanguage": number, "audienceEngagement": number }, "dimensionCommentary": { "voicePacing": { "whatIsWorking": string, "needsImprovement": string, "coachingRecommendation": string }, "presenceConfidence": { "whatIsWorking": string, "needsImprovement": string, "coachingRecommendation": string }, "bodyLanguage": { "whatIsWorking": string, "needsImprovement": string, "coachingRecommendation": string }, "audienceEngagement": { "whatIsWorking": string, "needsImprovement": string, "coachingRecommendation": string } }, "topStrengths": string[], "topPriorityFixes": string[], "coachingMoments": [{ "timestamp": string, "startSec": number, "endSec": number, "title": string, "observation": string, "whyItMatters": string, "coachingTip": string, "severity": "low" | "medium" | "high" }], "practicePlan": [{ "focusArea": string, "exercise": string, "frequency": string, "goal": string }], "processingNotes": { "transcriptConfidence": string, "visualConfidence": string, "limitations": string[] } }',
    "Always include dimensionCommentary for all four dimensions.",
    "For each dimension, state clearly: what is working, what needs improvement, and one specific coaching recommendation.",
