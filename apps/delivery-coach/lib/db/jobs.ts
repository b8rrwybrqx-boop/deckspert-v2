...
export async function appendProcessingEvent(
  jobId: string,
  stage: DeliveryJobStatus | string,
  message: string,
  metadataJson?: PrismaTypes.JsonObject
) {
...
export async function updateDeliveryJobStatus(
  ...
  extras?: Partial<{
    ...
    completedAt: Date;
    failedAt: Date;
    processingLogs: PrismaTypes.JsonValue;
  }>
) {
  return prisma.deliveryJob.update({
    ...
    data: {
      ...
      processingLogs:
        extras && "processingLogs" in extras ? extras.processingLogs ?? Prisma.JsonNull : undefined
    }
  });
}
...
export async function createDerivedAsset(
  ...
  blobUrl: string,
  metadataJson?: PrismaTypes.JsonValue
) {
