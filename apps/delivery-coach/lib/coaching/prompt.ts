...
export function buildCoachingPrompt(input: BuildPromptInput) {
  ...
  return [
    ...
    '{ "executiveSummary": string, "overallScore": number, "dimensionScores": { "voicePacing": number, "presenceConfidence": number, "bodyLanguage": number, "audienceEngagement": number }, "topStrengths": string[], "topPriorityFixes": string[], "coachingMoments": [{ "category": "delivery" | "clarity" | "confidence" | "pacing" | "fillerWords" | "pausing" | "structure" | "bodyLanguage" | "audienceEngagement", "timestamp": string, "startSec": number, "endSec": number, "title": string, "observation": string, "whyItMatters": string, "coachingTip": string, "severity": "low" | "medium" | "high" }], "practicePlan": [{ "focusArea": string, "exercise": string, "frequency": string, "goal": string }], "processingNotes": { "transcriptConfidence": string, "visualConfidence": string, "limitations": string[] } }',
    "Return valid JSON only.",
    "If a category appears in the summary, score discussion, or priority fixes, it must also appear in coachingMoments.",
    "If evidence is insufficient for a category such as bodyLanguage, still include the category and say insufficient signal rather than omitting it.",
    "",
    "User context:",
