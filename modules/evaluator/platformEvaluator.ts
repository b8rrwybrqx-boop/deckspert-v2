...
export const PLATFORM_EVALUATOR_SYSTEM_PROMPT = `# Deckspert™ Evaluator v4.5 — Claude Project System Prompt
...
5. Page-by-Page Evaluation (tables grouped in sets of five slides)
...
### Section-Level Scoring Definitions (apply exactly)
...
- Simplicity (Score + 1–2 sentences)
- Ease of Understanding (Score + 1–2 sentences)
- Visual Appeal (Score + 1–2 sentences)
- Readability (Score + 1–2 sentences)
- Title Effectiveness (Score + 1–2 sentences)
- Strengths (bullets)
- Weaknesses (bullets)
- Opportunities (1–3 bullets)

If a slide has no visible title, assign a short functional placeholder (e.g., "Traffic Trend Slide").

### Slide-Level Scoring Definitions (apply exactly)
...
- [ ] All nine story elements are evaluated and scored
- [ ] All three Section-Level tables are complete (3 rows each)
- [ ] Overall Story Evaluation table is complete (2 rows)
- [ ] Page-by-page composite tables are grouped correctly (sets of five)
...
**Final footer on every evaluation:**
...
*End of Deckspert™ Evaluator v4.5 System Prompt*`;

// Override via PLATFORM_EVALUATOR_MODEL env var; defaults to gpt-4.1 for full evaluation quality.
// claude-sonnet-4-6 handles the strict v4.5 rubric and long structured output well.
// Override via PLATFORM_EVALUATOR_MODEL env var; use claude-opus-4-7 for maximum quality.
const DEFAULT_MODEL = "claude-sonnet-4-6";
const ANTHROPIC_API_URL = "https://api.anthropic.com/v1/messages";
const ANTHROPIC_VERSION = "2023-06-01";
const MAX_TOKENS = 16000;

function buildUserMessage(artifacts: Artifact[], notes: string): string {
  const parts: string[] = [];
...
export async function runPlatformEvaluation(input: {
  ...
  const userMessage = buildUserMessage(input.artifacts, input.notes);
  ...
  const response = await fetch(ANTHROPIC_API_URL, {
    ...
    body: JSON.stringify({
      ...
      system: PLATFORM_EVALUATOR_SYSTEM_PROMPT,
