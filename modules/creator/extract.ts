...
function inferProperPrepStructure(text: string) {
  ...
  const coreNeeds =
    needs.core.length > 0
      ? needs.core
      : extractSectionItems(source, ["core (dept/category) needs", "core needs"], ["business needs", "personal needs", "desired outcome"]);
  const businessNeeds =
    needs.business.length > 0
      ? needs.business
      : extractSectionItems(source, ["business needs"], ["personal needs", "desired outcome", "reasons to say yes"]);
  const personalNeeds =
    needs.personal.length > 0
      ? needs.personal
      : extractSectionItems(source, ["personal needs"], ["desired outcome", "reasons to say yes", "reasons to say no"]);
